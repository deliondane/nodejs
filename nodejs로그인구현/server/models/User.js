...
const userSchema = mongoose.Schema({
  ...
  role: {
    // 예) 넘버가 1이면 관리자고 넘버가 0이면 일반유저
    type: Number,
    default: 0,
  },
  image: String,
  token: {
    // 토큰을 이용해 나중에 유효성 관리를 할 수 있음
    type: String,
  },
  tokenExp: {
    //토큰을 사용할 수 있는 기간
...
//mongoose의 pre()메서드를 활용'save'메서드가 호출되기 전에 콜백함수가 실행
userSchema.pre("save", function (next) {
  //인자로 전달된 next는 pre메서드가 실행된 후 다시 save메서드가 호출된 위치로 실행을 넘기기 위해서 필요
  //this = userSchema
  const user = this;
...
//토큰을 생성위해 generateToken메서드를 생성(메서드 이름은 변경해도 됨)
